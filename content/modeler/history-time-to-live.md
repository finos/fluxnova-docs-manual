...
Each execution of a model resource (BPMN, DMN, and CMMN) generates historic data during execution that is stored in the database. In Fluxnova, [history cleanup]({{< relref "/user-guide/process-engine/history.md" >}}) removes this historic data from the database based on a defined **history time to live (HTTL)**.
...
Refer to the [Fluxnova documentation regarding case definitions]({{< relref "/user-guide/process-engine/history.md#process-decision-case-definitions" >}}) for additional details.
...
- [History documentation]({{< relref "/user-guide/process-engine/history.md" >}})
...
This document describes the installation of Fluxnova and its components on a vanilla [WildFly Application Server](http://www.wildfly.org) or JBoss EAP 7 / 8.
...
Copy the modules from the `modules/` folder of the Fluxnova distribution, or extract the `fluxnova-wildfly-modules` archive, to the `$WILDFLY_HOME/modules/` of your WildFly application server.
...
The version shipped with Fluxnova is the version that the process engine is tested on and it is strongly recommended to use Fluxnova's version.
...
Add the Fluxnova subsystem as extension:

```xml
<server xmlns="urn:jboss:domain:20.0">
  <extensions>
    ...
    <extension module="org.finos.fluxnova.bpm.wildfly.fluxnova-wildfly-subsystem"/>
```
...
This section describes how to install optional dependencies. None of these are required to work with the core platform. Before continuing, make sure that Fluxnova is already installed according to [this step]({{< relref "#setup" >}}) for WildFly / JBoss EAP.
...
4. Startup WildFly.
5. Access Cockpit, Tasklist, and Admin via `/fluxnova/app/cockpit`, `/fluxnova/app/tasklist` and `/fluxnova/app/admin`, or under the context path you configured.
...
* `org/finos/fluxnova/bpm/fluxnova-engine-plugin-connect`
* `org/finos/fluxnova/commons/fluxnova-commons-utils`

To activate Fluxnova Connect functionality for a process engine, a process engine plugin has to be registered in `$WILDFLY_HOME/standalone/configuration/standalone.xml` as follows:
...
You can use the Fluxnova Spin plugin to extend the engine functionality to de-/serialize object variables from and to JSON and XML. For more information, see the [Spin Reference]({{< relref "/reference/spin/_index.md" >}}).
...
In order to activate Fluxnova Spin functionality for a process engine, a process engine plugin has to be registered in `$WILDFLY_HOME/standalone/configuration/standalone.xml` as follows:
  ...
  <process-engines>
    <process-engine name="default" default="true">
      ...
      <plugins>
        ... existing plugins ...
        <plugin>
          <class>org.finos.fluxnova.spin.plugin.impl.SpinProcessEnginePlugin</class>
        </plugin>
      </plugins>
      ...
    </process-engine>
...
The usage of Jackson annotations on WildFly together with the Fluxnova Spin JSON serialization can lead to problems.
...
This JAX-RS subsystem includes the Jackson library, the version of which does not match with the version used by the Fluxnova SPIN Plugin.
...
The Spin plugin also comes into play when JSON variables are set or read by the Fluxnova Process Engine.

See one of the following ways to fix this:

1. Change the Jackson `main` slot to the version which is used by the Fluxnova Spin Plugin.
...
Fluxnova Spin is not available in scripts if a process definition is deployed via REST API. Because WildFly handles dependencies using its module system and Fluxnova engine module has no module dependency on the spin module.
...
* `org/finos/fluxnova/template-engines/fluxnova-template-engines-freemarker`
* `org/freemarker/freemarker`
* `org/finos/fluxnova/commons/fluxnova-commons-logging`
* `org/finos/fluxnova/commons/fluxnova-commons-utils`

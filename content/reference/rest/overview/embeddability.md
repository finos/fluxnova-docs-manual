...
The REST API classes are tested with [RESTEasy](https://resteasy.dev/) and [Jersey](https://eclipse-ee4j.github.io/jersey/) as the JAX-RS implementation.
...
public class MyApplication extends Application {
  ...
  public Set<Class<?>> getClasses() {
    ...
    ...
    // add all fluxnova engine rest resources (or just add those that you actually need).
    classes.addAll(FluxnovaRestResources.getResourceClasses());

    // mandatory
    classes.addAll(FluxnovaRestResources.getConfigurationClasses());
...
`org.finos.fluxnova.bpm.engine.rest.exception.JsonMappingExceptionHandler` and `org.finos.fluxnova.bpm.engine.rest.exception.JsonParseExceptionHandler`.
...
For proper exception responses of the format as described in the [Introduction]({{< relref "/reference/rest/overview/_index.md" >}}),
...
If you would like to have all kinds of exceptions translated to this format, you can use `org.finos.fluxnova.bpm.engine.rest.exception.ExceptionHandler` instead of `ProcessEngineExceptionHandler`.
...
and declare it in a file `META-INF/services/org.finos.fluxnova.bpm.engine.rest.spi.ProcessEngineProvider`.
You may also declare the class `org.finos.fluxnova.bpm.engine.rest.impl.application.ContainerManagedProcessEngineProvider`
which comes with the REST API and uses the methods that the class `org.finos.fluxnova.bpm.BpmPlatform` provides.

...
title: 'The Fluxnova WildFly Subsystem'
weight: 50
...
{{< note title="Installation Guide" class="info" >}}
  ...
  [Read the installation guide]({{< relref "/installation/full/jboss/_index.md" >}}) to learn how to install the Fluxnova Wildfly subsystem into your Wildfly Server.
{{< /note >}}

Fluxnova provides advanced integration for Wildfly in the form of a custom [Wildfly Subsystem](https://docs.wildfly.org/23/Extending_WildFly.html).
...
* Use a managed Thread Pool for the Job Executor configured through the Fluxnova Subsystem.

# Configure the Job Executor in standalone.xml/domain.xml

The configuration of the thread pool used by the Job Executor is done in the Fluxnova subsystem, which also controls the thread pool creation and shutdown.  
...
For a complete list of all configuration options, please refer to the [Job Executor Configuration]({{< relref "/reference/deployment-descriptors/tags/job-executor.md" >}}).

# Configure a Process Engine in standalone.xml/domain.xml



Using the Fluxnova Wildfly subsystem, it is possible to configure and manage the process engine through the JBoss Management Model. The most straightforward way is to add the process engine configuration to the `standalone.xml` file of the Wildfly Server:
...
For a complete list of all configuration options, please refer to the [Process Engine Configuration]({{< relref "/reference/deployment-descriptors/tags/process-engine.md" >}}).
...
The class `org.my.custom.ProcessEngineConfiguration` must be a subclass of `org.finos.fluxnova.bpm.engine.impl.cfg.JtaProcessEngineConfiguration`.
...
{{< note title="Module dependency of custom configuration class" class="warning" >}}
  If you configure the process engine in `standalone.xml` and provide a custom configuration class packaged inside an own module, the fluxnova-wildfly-subsystem module needs to have a module dependency on the module providing the class.

  If you fail to do this, you will see the following error log:

  ```console
  Caused by: org.finos.fluxnova.bpm.engine.ProcessEngineException: Could not load 'foo.bar': the class must be visible from the fluxnova-wildfly-subsystem module.
      at org.finos.fluxnova.bpm.container.impl.jboss.service.MscManagedProcessEngineController.createProcessEngineConfiguration(MscManagedProcessEngineController.java:187) [fluxnova-wildfly-subsystem-{{< minor-version >}}.0.jar:]
      at org.finos.fluxnova.bpm.container.impl.jboss.service.MscManagedProcessEngineController.startProcessEngine(MscManagedProcessEngineController.java:138) [fluxnova-wildfly-subsystem-{{< minor-version >}}.0.jar:]
      at org.finos.fluxnova.bpm.container.impl.jboss.service.MscManagedProcessEngineController$3.run(MscManagedProcessEngineController.java:126) [fluxnova-wildfly-subsystem-{{< minor-version >}}.0.jar:]
  ```
...
<subsystem xmlns="urn:org.finos.fluxnova.bpm.jboss:1.1">
    <process-engines>
        <process-engine name="default" default="true">
            ...
            <plugins>
                <plugin>
                    <class>org.finos.fluxnova.bpm.engine.MyCustomProcessEnginePlugin</class>
                    <properties>
 ...
 * Plugin class must be visible in the classpath for the Fluxnova subsystem.
...
The Fluxnova Wildfly subsystem provides the same [JNDI bindings for the ProcessApplicationService and the ProcessEngineService]({{< relref "/user-guide/runtime-container-integration/jndi-bindings-for-bpmn-platform-services.md" >}}) as provided on other containers. In addition, the Fluxnova Wildfly subsystem creates JNDI Bindings for all managed process engines, allowing us to look them up directly.

The global JNDI bindings for process engines follow the pattern

```java
java:global/fluxnova-bpm-platform/process-engine/$PROCESS_ENGINE_NAME
```

If a process engine is named "engine1", it will be available using the name `java:global/fluxnova-bpm-platform/process-engine/engine1`.
...
[standalone@localhost:9999 /] cd /subsystem=fluxnova-bpm-platform
[standalone@localhost:9999 subsystem=fluxnova-bpm-platform] :read-resource(recursive=true)
{
    "outcome" => "success",
    "result" => {
        ...
        "process-engines" => {"default" => {
            "configuration" => "org.finos.fluxnova.bpm.container.impl.jboss.config.ManagedJtaProcessEngineConfiguration",
...
```console
[standalone@localhost:9999 subsystem=fluxnova-bpm-platform] cd process-engines=default
...
```console
[standalone@localhost:9999 subsystem=fluxnova-bpm-platform] /subsystem=fluxnova-bpm-platform/process-engines=my-process-engine/:add(name=my-process-engine,datasource=java:jboss/datasources/ExampleDS)
{"outcome" => "success"}
```
...
When using the Fluxnova Wildfly subsystem, the process engine classes are deployed as WildFly module. The module is named
`org.finos.fluxnova.bpm.fluxnova-engine` and is deployed in the folder `$WILDFLY_HOME/modules/org/finos/fluxnova/bpm/fluxnova-engine`.
...
When using the Process Application API (i.e., when deploying either a servlet process application or an EJB process application), the Fluxnova Wildfly subsystem will detect the `@ProcessApplication` class in the deployment and automatically add a module dependency between the application and the process engine module. As a result, we don't have to declare the dependency ourselves. It is called an [implicit module dependency](https://docs.wildfly.org/23/Developer_Guide.html#Implicit_module_dependencies_for_deployments) because it is not explicitly declared but can be derived by inspecting the application and seeing that it provides a `@ProcessApplication` class.
   ...
   <plugins>
     <plugin>
       ...
       <configuration>
          <archive>
             <manifestEntries>
                <Dependencies>org.finos.fluxnova.bpm.fluxnova-engine</Dependencies>
             </manifestEntries>
          </archive>
       </configuration>
     </plugin>
...
The Fluxnova Wildfly subsystem manages process engines as JBoss Services in the JBoss Module Service Container. For the Module Service Container to provide the process engine service(s) to the deployed applications, it is important that the dependencies are known. Consider the following example:
...
When using the Process Application API (i.e., when deploying either a servlet process application or an EJB process application), the Fluxnova Wildfly subsystem will detect the `@ProcessApplication` class in the deployment and automatically add a service dependency between the process application component and the process engine module. This ensures that the process engine is available when the process application is deployed.
...
<resource-ref>
  ...
  <res-type>org.finos.fluxnova.bpm.engine.ProcessEngine</res-type>
  <mapped-name>java:global/fluxnova-bpm-platform/process-engine/default</mapped-name>
</resource-ref>
```

This way, the global process engine resource `java:global/fluxnova-bpm-platform/process-engine/default` is available locally under the name `processEngine/default`. Since the application server is aware of this dependency, it will make sure the Process Engine Service exists before starting the application and it will stop the application if the process engine is removed.
...
public class PaComponent {
  ...
  @Resource(mappedName="java:global/fluxnova-bpm-platform/process-engine/default")

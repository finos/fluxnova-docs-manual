...
The following sections go into the details of integrating Spin with the process engine. Note that when you use a pre-built Fluxnova distribution, Spin is already integrated.
...
`fluxnova-spin-core` is a jar that contains only the core Spin classes. It can be combined with single data format artifacts. Fluxnova provides the artifacts `fluxnova-spin-dataformat-json-jackson` and `fluxnova-spin-dataformat-xml-dom` (`fluxnova-spin-dataformat-xml-dom-jakarta` for Jakarta XML Binding 4.0 support) that implement JSON and XML processing. These artifacts transitively pull in libraries they need. For example, `fluxnova-spin-dataformat-json-jackson` has a dependency to `jackson-databind`.

## fluxnova-spin-dataformat-all

`fluxnova-spin-dataformat-all` is a fat jar that contains `fluxnova-spin-core`, `fluxnova-spin-dataformat-json-jackson` and `fluxnova-spin-dataformat-xml-dom` as well as all their dependencies. The dependencies are shaded into the `spinjar` package namespace.

Note that the package relocation means that you cannot develop against the original namespaces. Example: `fluxnova-spin-dataformat-json-jackson` uses `jackson-databind` for object (de-)serialization. A common use case is declaring Jackson annotations in custom classes to finetune JSON handling. With relocated dependencies, annotations in the `com.fasterxml.jackson` namespace will not be recognized by Spin. In that case, consider using `fluxnova-spin-core`. Keep in mind the implications this may have as described in the [Integration Use Cases](#integration-use-cases) section.

## fluxnova-engine-plugin-spin

`fluxnova-engine-plugin-spin` is a process engine plugin that integrates Spin with a process engine. For example, it 
...
Depending on the application and process engine setup, it is recommended to use either `fluxnova-engine-plugin-spin` and `fluxnova-spin-core` (plus individual data formats) or `fluxnova-engine-plugin-spin` and `fluxnova-spin-dataformat-all`. The following sections explain when to use which for the most common use cases.
...
Add the dependencies to `fluxnova-engine-plugin-spin` and `fluxnova-spin-core` (along with `fluxnova-spin-dataformat-json-jackson` and `fluxnova-spin-dataformat-xml-dom` as needed) to your application. If you need to use Jakarta XML Binding 4.0 (e.g. Springboot version 3.x.x), use `fluxnova-spin-dataformat-xml-dom-jakarta` instead of `fluxnova-spin-dataformat-xml-dom`.
...
* Tomcat: `fluxnova-spin-dataformat-all` is provided in Tomcat's shared library path. Using `fluxnova-spin-dataformat-all` avoids classpath pollution with Spin's dependencies. For example, this ensures that applications are not forced to use Spin's version of Jackson.
* Wildfly: `fluxnova-spin-core` (along with `fluxnova-spin-dataformat-json-jackson` and `fluxnova-spin-dataformat-xml-dom`) are deployed as modules. Thanks to Wildfly's module system, classpath pollution is not an issue. Whenever a process application is deployed, it receives an implicit module dependency to `fluxnova-spin-core`.
...
* Make sure to depend on the same Spin artifacts that the application server provides, i.e. either `fluxnova-spin-core` or `fluxnova-spin-dataformat-all`.

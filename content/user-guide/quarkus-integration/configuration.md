...
This section of the Fluxnova Quarkus extension documentation covers the configuration options for the process engine
within a Quarkus application.

The documentation on the Fluxnova Quarkus Extension Configuration is intended for Quarkus application developers. It 
requires some knowledge on [Quarkus CDI support][quarkus-cdi], [Quarkus configuration][quarkus-config], as well as
Fluxnova Process Engine Configuration properties.
...
<table class="table desc-table">
  ...
  <tr>
    ...
    <td>
      The <a href="{{< relref "/user-guide/process-engine/database/database-configuration.md#example-database-configuration" >}}">Database Configuration</a> 
  ...
  <tr>
    ...
    <td>
      An instance of {{< javadocref page="org/finos/fluxnova/bpm/engine/impl/persistence/StrongUuidGenerator.html" text="StrongUuidGenerator" >}}
      is used.
    </td>
  ...
  <tr>
    ...
    <td>
      Fluxnova Cockpit works best with history level 
      <a href="{{< relref "/user-guide/process-engine/history/history-configuration.md#choose-a-history-level">}}">FULL</a>.
    </td>
...
configuring a Quarkus application in the [Quarkus configuration][quarkus-config] page. The Fluxnova Quarkus extension 
docs use the `application.properties` format in the examples, but you can use any supported Quarkus config source.

You can set any process engine configuration properties under the `quarkus.fluxnova` prefix. The 
...
```properties
quarkus.fluxnova.generic-config.process-engine-name=quarkusEngine
```
...
configuration properties under the `quarkus.fluxnova.job-executor` prefix. The [Job Executor Configuration Properties][executor-properties] 
...
sections, the Fluxnova Quarkus extension provides some Quarkus-specific configuration properties. They can be set
...
<table class="table desc-table">
  ...
  <tr>
    <td rowspan="1"><code>quarkus.fluxnova</code></td>
  ...
  <tr>
    <td rowspan="2"><code>quarkus.fluxnova.job-executor.thread-pool</code></td>
...
A datasource is required to run the Fluxnova process engine.
...
should use by its name via the `fluxnova.datasource` configuration property. Consider the example configuration below:
...
quarkus.fluxnova.datasource=engine-datasource
```
...
[engine-properties]: {{< relref "/reference/deployment-descriptors/tags/process-engine.md#configuration-properties" >}}
[executor-properties]: {{< relref "/reference/deployment-descriptors/tags/job-executor.md#job-acquisition-configuration-properties" >}}
...
[jta-transaction-integration]: {{< relref "/user-guide/cdi-java-ee-integration/jta-transaction-integration.md">}}

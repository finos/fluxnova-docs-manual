...
To use the global process application Event Listeners, you need to activate the corresponding [Process Engine Plugin]({{< relref "/user-guide/process-engine/process-engine-plugins.md" >}}):
    ...
    <process-engine name="default">
      ...
      <plugins>
        <plugin>
          <class>org.finos.fluxnova.bpm.application.impl.event.ProcessApplicationEventListenerPlugin</class>
        </plugin>
      </plugins>
    </process-engine>

Note that the plugin is activated by default in the pre-packaged Fluxnova distributions.

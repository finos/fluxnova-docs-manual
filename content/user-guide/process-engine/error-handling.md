...
If you don't want the exception being shown to the user, one option is to make service calls, which might cause an error, async (as described in [Transactions in Processes]({{< relref "/user-guide/process-engine/transactions-in-processes.md" >}})). In that case the exception is stored in the process engine database and the [Job]({{< relref "/user-guide/process-engine/the-job-executor.md" >}}) in the background is marked as failed (to be more precise, the exception is stored and some retry counter is decremented).
...
of the type `{{< javadocref page="org/finos/fluxnova/bpm/engine/ProcessEngineException.html" text="ProcessEngineException" >}}`.
...
You can access error codes via Java as well as [REST API]({{< relref "/reference/rest/overview/_index.md#exception-codes" >}}).
...
Sometimes you may want to assign codes to specific errors Fluxnova hasn't covered so far.
...
Learn more on how to assign a custom error code to an exception in the documentation about [Delegation Code]({{< relref "/user-guide/process-engine/delegation-code.md#exception-codes" >}}).

## Configuration

You can configure the exception error codes feature in your [process engine configuration]({{< relref "/reference/deployment-descriptors/tags/process-engine.md#exception-codes" >}}):
...
With the help of a [`ProcessEnginePlugin`]({{< relref "/user-guide/process-engine/process-engine-plugins.md" >}}) you can register a custom `{{< javadocref page="org/finos/fluxnova/bpm/engine/impl/errorcode/ExceptionCodeProvider.html" text="ExceptionCodeProvider" >}}`:

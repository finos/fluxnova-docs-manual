...
Fluxnova supports the Unified Expression Language (EL), specified by the [Jakarta Expression
...
Within Fluxnova, EL can be used in many circumstances to evaluate small script-like
...
Besides Java code, Fluxnova also supports the evaluation of expressions as delegation code. For
...
Two types of expressions are currently supported: `fluxnova:expression` and
`fluxnova:delegateExpression`.

With `fluxnova:expression` it is possible to evaluate a value expression or to invoke
...
```xml
  <process id="process">
    <extensionElements>
      ...
      <fluxnova:executionListener event="start" expression="${execution.setVariable('test', 'foo')}" />
    </extensionElements>
    ...
    <userTask id="userTask">
      <extensionElements>
        ...
        <fluxnova:taskListener event="complete" expression="${myBean.taskDone(task)}" />
      </extensionElements>
    </userTask>
    ...
    <serviceTask id="serviceTask"
        fluxnova:expression="${myBean.ready}" fluxnova:resultVariable="myVar" />
...
The attribute `fluxnova:delegateExpression` is used for expressions which evaluate to a delegate
...
```xml
  ...
  <serviceTask id="task1" fluxnova:delegateExpression="${myBean}" />

  <!-- service task which calls a method which returns delegate object -->
  <serviceTask id="task2" fluxnova:delegateExpression="${myBean.createDelegate()}" />
```
...
```xml
  <serviceTask id="task" fluxnova:class="org.finos.fluxnova.bpm.example.SumDelegate">
    <extensionElements>
      <fluxnova:inputOutput>
        <fluxnova:inputParameter name="x">
          ${myBean.calculateX()}
        </fluxnova:inputParameter>
      </fluxnova:inputOutput>
    </extensionElements>
  </serviceTask>
```
...
[fluxnova:errorEventDefinition]({{< relref "/reference/bpmn20/custom-extensions/extension-elements.md#erroreventdefinition" >}})
...
[Error Boundary Event]({{< relref "/reference/bpmn20/events/error-events.md#error-boundary-event" >}}).

In the scope of an External Task, expressions have access to the {{< javadocref page="org/finos/fluxnova/bpm/engine/externaltask/ExternalTask.html" text="ExternalTaskEntity" >}} object via the key `externalTask` which provides getter methods for `errorMessage`, `errorDetails`, `workerId`, `retries` and more.
...
```xml
<bpmn:serviceTask id="myExternalTaskId" name="myExternalTask" fluxnova:type="external" fluxnova:topic="myTopic">
  <bpmn:extensionElements>
    <fluxnova:errorEventDefinition id="myErrorEventDefinition" errorRef="myError" expression="${externalTask.getWorkerId() == 'myWorkerId'}" />
  </bpmn:extensionElements>
</bpmn:serviceTask>
```
...
<bpmn:serviceTask id="myExternalTaskId" name="myExternalTask" fluxnova:type="external" fluxnova:topic="myTopic">
  <bpmn:extensionElements>
    <fluxnova:errorEventDefinition id="myErrorEventDefinition" errorRef="myError" expression="${externalTask.getErrorDetails().contains('myErrorMessage')}" />
  </bpmn:extensionElements>
</bpmn:serviceTask>
```

For further details on the functionality of error event definitions in the context of external tasks, consult the [External Tasks Guide]({{< relref "/user-guide/process-engine/external-tasks.md#error-event-definitions" >}}).
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td><code>{{< javadocref page="org/finos/fluxnova/bpm/engine/delegate/DelegateExecution.html" text="DelegateExecution" >}}</code></td>
      <td>
    ...
    <tr>
      ...
      <td><code>{{< javadocref page="org/finos/fluxnova/bpm/engine/delegate/DelegateTask.html" text="DelegateTask" >}}</code></td>
      <td>Available in a task context like a task listener.</td>
    </tr>
    <tr>
      <td><code>externalTask</code></td>
      <td><code>{{< javadocref page="org/finos/fluxnova/bpm/engine/externaltask/ExternalTask.html" text="ExternalTask" >}}</code></td>
      <td>Available during an external task context activity (e.g. in <a href="{{< relref "/reference/bpmn20/custom-extensions/extension-elements.md#erroreventdefinition" >}}">fluxnova:errorEventDefinition</a> expressions).</td>
    </tr>
    <tr>
      <td><code>caseExecution</code></td>
      <td><code>{{< javadocref page="org/finos/fluxnova/bpm/engine/delegate/DelegateCaseExecution.html" text="DelegateCaseExecution" >}}</code></td>
...
```xml
  <fluxnova:executionListener event="start"
...
```xml
  <serviceTask id="task2" fluxnova:expression="${myBean.myMethod(execution)}" />
```
...
```xml
<userTask id="theTask" name="Important task" fluxnova:dueDate="${dateTime().plusDays(3).toDate()}"/>
```


## Built-In Fluxnova Spin Functions

If the Fluxnova Spin process engine plugin is activated, the Spin functions `S`,
...
```xml
  <serviceTask id="task" fluxnova:expression="${XML(xml).attr('test').value()}" resultVariable="test" />
```

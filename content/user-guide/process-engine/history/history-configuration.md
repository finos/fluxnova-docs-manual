...
It can also be set using Spring XML or a deployment descriptor (bpm-platform.xml, processes.xml). When using the Fluxnova Wildfly Subsystem, the property can be set through Wildfly configuration (standalone.xml, domain.xml).
...
Below are a few examples which show some of the possibilities of the query API for history. Full description of the possibilities can be found in the Javadocs, in the `org.finos.fluxnova.bpm.engine.history` package.
...
The supported periods so far are `MONTH` and `QUARTER` from `org.finos.fluxnova.bpm.engine.query.PeriodUnit`.
...
To work around this, the Fluxnova engine generates sequence numbers which can be used to *partially*

...
The following properties are available in the `org.finos.fluxnova.bpm.engine.ProcessEngineConfiguration` class:
...
```java
org.finos.fluxnova.bpm.engine.impl.cfg.ProcessEngineConfigurationImpl.setCustomIncidentHandlers(List<IncidentHandler>)
```
...
Please be aware that this API is **not** part of the [public API]({{< relref "/introduction/public-api.md" >}}) and may change in later releases.
...
{{< javadocref page="org/finos/fluxnova/bpm/engine/impl/incident/CompositeIncidentHandler.html" text="Composite Incident Handler" >}}.
...
Fluxnova distinguishes between read-only and writable user repositories. A read-only user repository provides read-only access to the underlying user/group database. A writable user repository allows write access to the user database which includes creating, updating and deleting users and groups.

To provide a custom identity provider implementation, the following interfaces can be implemented:

* {{< javadocref page="org/finos/fluxnova/bpm/engine/impl/identity/ReadOnlyIdentityProvider.html" text="org.finos.fluxnova.bpm.engine.impl.identity.ReadOnlyIdentityProvider" >}}
* {{< javadocref page="org/finos/fluxnova/bpm/engine/impl/identity/WritableIdentityProvider.html" text="org.finos.fluxnova.bpm.engine.impl.identity.WritableIdentityProvider" >}}

# Custom Whitelist for User, Group and Tenant IDs

User, Group and Tenant IDs can be matched against a Whitelist Pattern to determine if the provided ID is acceptable or not. The default (global) Regular Expression pattern to match against is **"[a-zA-Z0-9]+|fluxnova-admin"** i.e. any combination of alphanumeric values or _'fluxnova-admin'_.
...
Note that if a certain pattern isn't defined (ex. the tenant whitelist pattern), the general pattern will be used, either the default one (`"[a-zA-Z0-9]+|fluxnova-admin"`) or one defined in the configuration file.    
...
```xml
<dependency>
  <groupId>org.finos.fluxnova.bpm.identity</groupId>
  <artifactId>fluxnova-identity-ldap</artifactId>
</dependency>
```
...
<beans xmlns="http://www.springframework.org/schema/beans"
  ...
  <bean id="processEngineConfiguration" class="org.finos.fluxnova.bpm.engine.impl.cfg.StandaloneInMemProcessEngineConfiguration">
    ...
    <property name="processEnginePlugins">
      <list>
        <relref bean="ldapIdentityProviderPlugin" />
      </list>
    </property>
  </bean>
  <bean id="ldapIdentityProviderPlugin" class="org.finos.fluxnova.bpm.identity.impl.ldap.plugin.LdapIdentityProviderPlugin">
    <property name="serverUrl" value="ldap://localhost:3433/" />
    <property name="managerDn" value="uid=daniel,ou=office-berlin,o=fluxnova,c=org" />
    <property name="managerPassword" value="daniel" />
    <property name="baseDn" value="o=fluxnova,c=org" />
...
<process-engine name="default">
  ...
  <plugins>
    <plugin>
      <class>org.finos.fluxnova.bpm.identity.impl.ldap.plugin.LdapIdentityProviderPlugin</class>
      <properties>

        <property name="serverUrl">ldap://localhost:4334/</property>
        <property name="managerDn">uid=jonny,ou=office-berlin,o=fluxnova,c=org</property>
        <property name="managerPassword">s3cr3t</property>

        <property name="baseDn">o=fluxnova,c=org</property>
...
<table class="table table-striped">
  ...
  <tr>
    ...
    <td>
      ...
      <p><em>Example:</em> <code>o=fluxnova,c=org</code></p>
    </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>
      ...
      <p>
        ...
        Make sure that your currently used LDAP Server implements [RFC 2891](https://datatracker.ietf.org/doc/html/rfc2891).
      </p>
    </td>
  </tr>
  <tr>
...
See the Spring Security OAuth2 Integration's [OAuth2 Identity Provider]({{< relref "/user-guide/spring-boot-integration/spring-security.md#oauth2-identity-provider" >}}) documentation.

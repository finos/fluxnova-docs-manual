...
Authentication means verifying a user's identity against the Fluxnova web apps. 
...
The Fluxnova web apps correlate the authentication information against [authorizations]({{< relref "/user-guide/process-engine/authorization-service.md" >}}) to determine 
...
Read the security implications of the authentication cache in our [Security Instructions]({{< relref "/user-guide/security.md#authentication-cache" >}}).
...
You can find the configuration properties for the Spring Boot Starter in the [User Guide]({{< relref "/user-guide/spring-boot-integration/configuration.md#auth-cache" >}}).
...
<filter>
  ...
  <filter-class>org.finos.fluxnova.bpm.webapp.impl.security.auth.AuthenticationFilter</filter-class>
  <init-param>
...
Fluxnova supports a broad range of containers, including Tomcat, Wildfly, IBM WebSphere and Oracle WebLogic. Using Container-Based Authentication, it is possible to move the authentication action to the container level, which will then make the authentication result available to the Fluxnova Web Applications.
...
```xml
  ...
  <filter>
    ...
    <filter-class>org.finos.fluxnova.bpm.webapp.impl.security.auth.ContainerBasedAuthenticationFilter</filter-class>
    <init-param>
      <param-name>authentication-provider</param-name>
      <param-value>org.finos.fluxnova.bpm.engine.rest.security.auth.impl.ContainerBasedAuthenticationProvider</param-value>
    </init-param>
  </filter>
  <filter-mapping>
    <filter-name>Container Based Authentication Filter</filter-name>
...
The Fluxnova Web Applications can also integrate with a [Single Sign-On implementation](https://en.wikipedia.org/wiki/List_of_single_sign-on_implementations) when the Container-Based Authentication servlet filter is enabled.
